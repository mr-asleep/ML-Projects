...
if __name__ == "__main__":
	data= pd.read_csv('/home/ritik/Downloads/2017EE104821.csv', header= None)
	data= np.array(data)
	X, Y= data[:,:25], data[:,25]
	# data= scale(data)
	# print(np.amax(data[:,:25]))
	# data= binary_classfication(data, 0, 1)
	train_X, test_X, train_Y, test_Y= train_test_split(X, Y, test_size= 0.2,stratify=data[:,25], random_state=42)
